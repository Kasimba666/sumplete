...
export default {
  name: 'Home',
  components: {BSResearch, Solutions},
  data() {
    ...
    return {
      ...
      game: {
        ...
        stepCounter: 0,
        steps: [], //массив объектов: x, y, status_before, status_after
        ...
        addStep: function (newX, newY, newSBefore, newSAfter) {
          this.steps.push({x: newX, y: newY, status_before: newSBefore, status_after: newSAfter});
          this.stepCounter++;
        },
        ...
        undoStep: function () {
          if (this.stepCounter > 0) {
            ...
            this.arrayField[step.y][step.x].status = step.status_before;
            this.owner.recalc();
            this.stepCounter--;
            this.owner.disableUndoRedo()
        ...
        redoStep: function () {
          if (this.stepCounter < this.steps.length) {
            ...
            this.arrayField[step.y][step.x].status = step.status_after;
            this.owner.recalc();
            this.stepCounter++;
  ...
  methods: {
    ...
    changeCellState(cell, i, j) {
      //отрезаем ходы от счётчика до конца массива Steps
//      this.game.steps = this.game.steps.slice(0, this.game.stepCounter);
      if (this.game.stepCounter < this.game.steps.length - 1) {
        this.game.steps = this.game.steps.slice(0, this.game.stepCounter + 1)
      }

      let status_before = cell.status;
      cell.status++;
      cell.status %= 3;
      let status_after = cell.status;
      this.game.addStep(i, j, status_before, status_after);
    ...
    createNewGame: function (sizeX, sizeY) {
      ...
      this.recalc();
      this.game.steps.push({x: 0, y: 0, status_before: 0, status_after: 0});
      ...
      if (this.game.arraySumHorizontal[y].status == 1) {
        ...
        for (index = 0; index < this.game.fieldSizeX(); index++) {
          if (this.game.arrayField[y][index].status === 0) {
            let status_before = this.game.arrayField[y][index].status;
            this.game.arrayField[y][index].status = 2;
            let status_after = this.game.arrayField[y][index].status;
            this.game.addStep(index, y, status_before, status_after);
            // alert(status_before + ' ' + status_after);
          }
        }
      } else {
        let index = 0;
...
//!!!проверить равенство
      if (this.game.arraySumVertical[x].status == 1) {
        ...
        for (index = 0; index < this.game.fieldSizeY(); index++) {
          if (this.game.arrayField[index][x].status === 0) {
            let status_before = this.game.arrayField[index][x].status;
            this.game.arrayField[index][x].status = 2;
            let status_after = this.game.arrayField[index][x].status;
            this.game.addStep(x, index, status_before, status_after);
          }
    ...
    disableUndoRedo() {
      ...
      if (this.game.stepCounter === (this.game.steps.length - 1)) {
        this.$refs['redo'].disabled = true;
      } else {
        this.$refs['redo'].disabled = false;
      }
      if (this.game.stepCounter === 0) {
